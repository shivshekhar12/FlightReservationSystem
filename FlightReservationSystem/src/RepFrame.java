...
public class RepFrame extends JFrame {
    ...
    private void showFlightDialog(String flightNo, String airlineId,
        ...
        for (JTextField f : new JTextField[]{tfFlightNo,tfAirline,tfAircraft,tfFrom,
                tfTo,tfDep,tfArr,tfEcoFare,tfBizFare,tfFstFare,tfDays})
            f.setFont(mainFont);
        ...
        try {
            ...
            if (isEdit) {
                ...
                DBConnection.getStatement().executeUpdate(
                    ...
                    "dep_time,arr_time,is_international,base_economy_fare,base_business_fare,base_first_fare) " +
                    "VALUES ('" + fn + "','" + al + "'," + ac + ",'" + frm + "','" + too + "','" +
                    dep + "','" + arr + "'," + intl + "," + eco + "," + biz + "," + fst + ")");
            }
            for (String day : days) {
                String d = day.trim();
                if (!d.isEmpty())
