...
public class WaitlistTab {
    ...
    public static JPanel build() {
        final JTextField flightNoField  = new JTextField(10);
        final JTextField airlineField   = new JTextField(4);
        final JTextField dateField      = new JTextField(12);

        final String[] columns = {"Position", "Customer ID", "Customer Name", "Email", "Requested", "Notified"};
        final DefaultTableModel tableModel = new DefaultTableModel(columns, 0) {
            public boolean isCellEditable(int row, int col) { return false; }
        };
        final JTable waitlistTable = new JTable(tableModel);
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                try {
                    ResultSet rs = DBConnection.getStatement().executeQuery(
                        ...
                        "WHERE w.flight_no='" + flightNo + "' AND w.airline_id='" + airlineId +
                        "' AND w.dep_date='" + date + "' ORDER BY w.position");
                    int count = 0;
                    while (rs.next()) {
                        String notified;
                        ...
                        tableModel.addRow(new Object[]{
                            ...
                            rs.getString("join_date"), notified});
                        count++;
                    }
        ...
        topPanel.add(new JLabel("Flight No:"));  topPanel.add(flightNoField);
        topPanel.add(new JLabel("Airline ID:")); topPanel.add(airlineField);
