...
public class ManageAircraftTab {
    ...
    public static JPanel build(JFrame parentFrame) {
        String[] columns = {"Aircraft ID", "Airline", "Model", "Economy Seats", "Business Seats", "First Seats"};
        DefaultTableModel tableModel = new DefaultTableModel(columns, 0) {
            public boolean isCellEditable(int row, int col) {
                return false;
            }
        };
        JTable aircraftTable = new JTable(tableModel);
        ...
        addButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                Object[] fields = {"Aircraft ID:", idField, "Airline ID:", airlineField, "Model:", modelField, "Economy Seats:", ecoSeatsField, "Business Seats:", bizSeatsField, "First Seats:", fstSeatsField};
                int dialogResult = JOptionPane.showConfirmDialog(parentFrame, fields, "Add Aircraft", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
                if (dialogResult != JOptionPane.OK_OPTION) {
                    return;
                }
                try {
                    DBConnection.getStatement().executeUpdate("INSERT INTO Aircraft (aircraft_id,airline_id,model,economy_seats,business_seats,first_seats) VALUES (" + idField.getText().trim() + ",'" + airlineField.getText().trim().toUpperCase() + "','" + modelField.getText().trim() + "'," + ecoSeatsField.getText().trim() + "," + bizSeatsField.getText().trim() + "," + fstSeatsField.getText().trim() + ")");
        ...
        editButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (selectedRow < 0) {
                    messageLabel.setText("Select an aircraft.");
                    return;
                }
                ...
                Object[] fields = {"Model:", modelField, "Economy Seats:", ecoSeatsField, "Business Seats:", bizSeatsField, "First Seats:", fstSeatsField};
                int dialogResult = JOptionPane.showConfirmDialog(parentFrame, fields, "Edit Aircraft " + aircraftId, JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
                if (dialogResult != JOptionPane.OK_OPTION) {
                    return;
                }
                try {
    ...
    private static void loadAllAircraft(DefaultTableModel tableModel, JLabel messageLabel) {
        ...
        try {
            ...
            while (rs.next()) {
                tableModel.addRow(new Object[]{rs.getInt("aircraft_id"), rs.getString("airline_id"), rs.getString("model"), rs.getInt("economy_seats"), rs.getInt("business_seats"), rs.getInt("first_seats")});
            }
