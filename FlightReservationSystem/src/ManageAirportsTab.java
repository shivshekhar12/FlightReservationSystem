...
public class ManageAirportsTab {
    ...
    public static JPanel build(JFrame parentFrame) {
        String[] columns = {"Airport ID", "Name", "City", "Country"};
        DefaultTableModel tableModel = new DefaultTableModel(columns, 0) {
            public boolean isCellEditable(int row, int col) {
                return false;
            }
        };
        JTable airportsTable = new JTable(tableModel);
        ...
        addButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                int dialogResult = JOptionPane.showConfirmDialog(parentFrame, fields, "Add Airport", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
        ...
        editButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (selectedRow < 0) {
                    messageLabel.setText("Select an airport.");
                    return;
                }
                ...
                int dialogResult = JOptionPane.showConfirmDialog(parentFrame, fields, "Edit Airport " + airportId, JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
                if (dialogResult != JOptionPane.OK_OPTION) {
                    return;
                }
                try {
    ...
    private static void loadAllAirports(DefaultTableModel tableModel, JLabel messageLabel) {
        ...
        try {
            ResultSet rs = DBConnection.getStatement().executeQuery("SELECT airport_id, name, city, country FROM Airport ORDER BY airport_id");
            while (rs.next()) {
                tableModel.addRow(new Object[]{rs.getString("airport_id"), rs.getString("name"), rs.getString("city"), rs.getString("country")});
            }
