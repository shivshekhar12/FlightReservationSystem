...
public class BookForCustomerTab {
    ...
    public static JPanel build(JFrame parentFrame, int empId) {
        final Map<String, Integer> customerMap = new LinkedHashMap<>();

        final JTextField customerSearchField = new JTextField(20);
        final JComboBox<String> customerCombo = new JComboBox<>();
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                try {
                    ResultSet rs = DBConnection.getStatement().executeQuery(
                        "SELECT cust_id, name, email FROM Customer WHERE name LIKE '%" + searchText + "%' ORDER BY name");
                    while (rs.next()) {
        ...
        final JTextField dateField    = new JTextField(12);
        final JComboBox<String> tripTypeCombo = new JComboBox<>(new String[]{"One-Way", "Round-Trip"});
        final JTextField returnDateField = new JTextField(12);

        final String[] columns = {"Flight", "Airline", "From", "To", "Depart", "Arrive", "Economy $", "Business $", "First $"};
        final DefaultTableModel tableModel = new DefaultTableModel(columns, 0) {
            public boolean isCellEditable(int row, int col) { return false; }
        };
        final JTable flightsTable = new JTable(tableModel);
        ...
        bookButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (selectedRow < 0) { messageLabel.setText("Select a flight first."); return; }
        ...
        customerPanel.add(new JLabel("Search name:")); customerPanel.add(customerSearchField);
        customerPanel.add(findCustomerButton);
        customerPanel.add(new JLabel("Customer:")); customerPanel.add(customerCombo);
        ...
        searchPanel.add(new JLabel("From:")); searchPanel.add(fromField);
        searchPanel.add(new JLabel("To:"));   searchPanel.add(toField);
        searchPanel.add(new JLabel("Date (YYYY-MM-DD):")); searchPanel.add(dateField);
        searchPanel.add(tripTypeCombo);
        searchPanel.add(new JLabel("Return:")); searchPanel.add(returnDateField);
    ...
    private static void doRepBooking(JFrame parentFrame, int empId, int custId,
                                     String flightNo, String airlineId, String depDate,
                                     String tripType, String returnDate, JLabel messageLabel) {
        ...
        try {
            ...
            ResultSet fareResult = DBConnection.getStatement().executeQuery(
                "SELECT " + fareColumn + ", booking_fee FROM Flight " +
            ...
            DBConnection.getStatement().executeUpdate(
                ...
                "VALUES (" + custId + "," + empId + "," + fare + "," + bookingFee + ",'" + ticketType + "',0,NOW(),'active')");
