...
public class FlightsByAirportTab {
    ...
    public static JPanel build() {
        final JTextField airportField       = new JTextField(6);
        final JComboBox<String> directionCombo = new JComboBox<>(new String[]{"Departing & Arriving", "Departing Only", "Arriving Only"});

        final String[] columns = {"Direction", "Flight", "Airline", "From", "To", "Depart", "Arrive", "Days"};
        ...
        loadButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (airportId.isEmpty()) { messageLabel.setText("Enter an airport ID."); return; }
                try {
                    if (direction == 0 || direction == 1) {
                        ResultSet rs = DBConnection.getStatement().executeQuery(
                            "SELECT f.flight_no, f.airline_id, f.dep_airport_id, f.arr_airport_id, f.dep_time, f.arr_time, " +
                        ...
                        while (rs.next()) {
                            tableModel.addRow(new Object[]{"Departing",
                    ...
                    if (direction == 0 || direction == 2) {
                        ...
                        while (rs.next()) {
                            tableModel.addRow(new Object[]{"Arriving",
                                rs.getString("flight_no"), rs.getString("airline_id"),
                                rs.getString("dep_airport_id"), rs.getString("arr_airport_id"),
                                rs.getString("dep_time"), rs.getString("arr_time"),
                                rs.getString("days")});
        ...
        topPanel.add(new JLabel("Airport ID:")); topPanel.add(airportField);
        topPanel.add(directionCombo); topPanel.add(loadButton);
